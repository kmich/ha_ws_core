...
- **Windrose dashboard example updated for lovelace-windrose-card v2.x (PR #151):** the vanilla dashboard now uses `data_period.period_back: -24h` instead of the removed `hours_to_show: 24` option.
...
- **Entity prefix consistency (issue #134):** most sensors (temperature, humidity, pressure, wind, and the two Vigicrues river sensors) ignored the configured entity prefix on a fresh install and were created as `sensor.weather_station_*` instead of `sensor.{prefix}_*`, breaking the pre-built dashboards which expect a consistent prefix. The 2.6.2 fix for the related "renamed entities revert on restart" bug relied on `_attr_suggested_object_id`, which Home Assistant only honours as a raw object id when `has_entity_name` is False; with `has_entity_name = True` (used by every entity here) it's treated as `object_id_base` and gets prefixed with the device name ("Weather Station") instead. All platforms (sensor, binary_sensor, number, select, switch, weather, event) now set `entity_id` directly at construction time, which Home Assistant *does* use verbatim regardless of `has_entity_name`. This also let us drop the ad-hoc "rename back to the expected id on every restart" logic a few platforms had grown as a workaround - which had its own bug of silently reverting a user's manual rename on every Home Assistant restart. If you already have entities stuck under the wrong `weather_station_` prefix, this fix only applies to newly created entities; rename the existing ones (or remove and let them regenerate) to match.
- **Vigicrues auto-detect and station picker stuck on HTTP 206 (issue #133):** Hub'Eau's station-lookup endpoint routinely answers HTTP 206 (Partial Content) rather than 200 whenever the result set doesn't include every matching station - this is normal, not an error, and the same coordinator already accepted it for the observations endpoint. The auto-detect fetch and the config/options flow's station dropdown only accepted a strict 200, so auto-detect looped on a `Vigicrues: auto-detect HTTP 206` warning every ~15 minutes and never resolved a station, and the manual "Stations Vigicrues" picker only ever showed "Auto (nearest station)". Both now accept 206 alongside 200.
- **Rain rate sawtooth (issue #132):** rain rate was computed as the instantaneous delta since the previous ~60s poll cycle. When the underlying rain-total sensor only updates in discrete bucket-tip increments, a tip landing inside one poll cycle read as a brief, disproportionate spike (e.g. 56 mm/h from a single 0.2-1mm tip) that decayed back to zero before the next tip - repeating for as long as it rained. Rain rate is now averaged over a short (10-minute) sliding window of the same rain-total history already tracked for the 1h/24h accumulation sensors, so a steady rain reads as a steady rate instead of oscillating every cycle.
- **Config flow: optional sensor fields couldn't be cleared (issue #135):** the optional source-sensor pickers (and the named-room temp/humidity/CO₂ pickers) pre-filled auto-detected guesses using a hard `default=`, which Home Assistant's entity picker cannot be cleared back to empty - if the guess was wrong, unavailable, or the wrong type, the form couldn't be saved at all. These fields now use `description={"suggested_value": ...}` instead, which still shows the same pre-filled suggestion but leaves the field genuinely optional and clearable.
